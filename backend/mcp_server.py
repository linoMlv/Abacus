...
_current_association: ContextVar[Association | None] = ContextVar("_current_association", default=None)
...
TOOLS = [
    Tool(
        ...
        description="Get the current association's account information (name, email, balances).",
        inputSchema={"type": "object", "properties": {}},
    ),
    Tool(
        name="list_balances",
        description="List all balances (accounts) for the association with their current amounts.",
        inputSchema={"type": "object", "properties": {}},
    ),
    Tool(
        name="create_balance",
        ...
        inputSchema={
            "type": "object",
            "properties": {
                "name": {"type": "string", "description": "Balance name (e.g., 'Main Account')"},
                "initial_amount": {"type": "number", "description": "Initial amount in euros"},
            },
    ...
    Tool(
        ...
        description="List operations (transactions), optionally filtered by date range.",
        inputSchema={
            "type": "object",
            "properties": {
                "start_date": {"type": "string", "description": "ISO date, e.g. 2024-01-01T00:00:00"},
                "end_date": {"type": "string", "description": "ISO date, e.g. 2024-12-31T23:59:59"},
            },
        },
    ),
    Tool(
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "group": {"type": "string", "description": "Category (e.g., 'Supplies')"},
                "amount": {"type": "number", "description": "Amount in euros (positive)"},
                "type": {"type": "string", "enum": ["income", "expense"]},
                "date": {"type": "string", "description": "ISO date"},
                "invoice": {"type": "string", "description": "Optional invoice reference"},
            },
            "required": ["balance_id", "name", "description", "group", "amount", "type", "date"],
        },
    ),
    Tool(
        ...
        inputSchema={
            ...
            "required": ["operation_id", "balance_id", "name", "description", "group", "amount", "type", "date"],
        },
    ),
    Tool(
    ...
    with Session(engine) as session:
        if name == "get_account_info":
            return _serialize({
                ...
                "balances": [
                    {"id": b.id, "name": b.name, "initialAmount": b.initialAmount, "position": b.position}
        ...
        if name == "create_balance":
            ...
            return _serialize({"id": balance.id, "name": balance.name, "initialAmount": balance.initialAmount})
        ...
        if name == "list_operations":
            stmt = select(Operation).join(Balance).where(Balance.association_id == association.id)
            ...
            return _serialize([
                {"id": o.id, "name": o.name, "description": o.description, "group": o.group,
                 "amount": o.amount, "type": o.type.value, "date": o.date, "balance_id": o.balance_id, "invoice": o.invoice}
                for o in ops
            ])
        ...
        if name == "get_balance_operations":
            ...
            if not balance or balance.association_id != association.id:
                return _serialize({"error": "Balance not found or unauthorized"})
            ops = session.exec(
                select(Operation).where(Operation.balance_id == args["balance_id"]).order_by(Operation.date.desc())
            ).all()
            return _serialize([
                {"id": o.id, "name": o.name, "description": o.description, "group": o.group,
                 "amount": o.amount, "type": o.type.value, "date": o.date, "invoice": o.invoice}
                for o in ops
            ])
        ...
        if name == "create_operation":
            ...
            session.commit()
            session.refresh(op)
            return _serialize({"id": op.id, "name": op.name, "amount": op.amount, "type": op.type.value})
...
async def mcp_asgi_app(scope, receive, send):
    """ASGI app that authenticates via X-API-Key then delegates to the MCP session manager."""
    if scope["type"] != "http":
        return
    ...
    if not api_key_header:
        ...
        _log_mcp_request(request.method, 401, None, ip_address, user_agent, start_time, "mcp_auth_failed", "Missing X-API-Key header")
        return
    ...
    if not association:
        ...
        await response(scope, receive, send)
        _log_mcp_request(request.method, 401, None, ip_address, user_agent, start_time, "mcp_auth_failed", "Invalid API key")
        return
    ...
    try:
        ...
        _log_mcp_request(request.method, captured_status[0], association.name, ip_address, user_agent, start_time, "mcp_request")
