...
from models import Association, Balance, BalanceRead, AssociationRead, association_to_read
...
def login(response: Response, request: LoginRequest, session: Session = Depends(get_session)):
    statement = select(Association).where(Association.name == request.name)
