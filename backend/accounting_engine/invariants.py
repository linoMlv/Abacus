"""The invariants that keep every entry balanced.

Intentionally pure (no I/O, no session): the balance invariant (Σ débit =
Σ crédit) can be unit-tested in isolation and reused by every write path (simple
entry, manual entry, import, recurrence) so no entry can ever be persisted
unbalanced.
"""
...
from models import Ecriture, EcritureOrigine, EcritureStatut, LigneEcriture
...
def exclude_cloture():
    """SQL clause excluding the result-determination entry from class-6/7 sums.

    The clôture entry (origine CLOTURE) zeroes the charge/produit accounts into
    the result account (12); counting it in an income statement would cancel the
    period's real result. Balance-sheet sums are unaffected (it never touches
    class 1-5 beyond the 12 account, which income statements exclude anyway).
    """
    return Ecriture.origine != EcritureOrigine.CLOTURE
