...
const PDFDocument: React.FC<PDFDocumentProps> = ({
  ...
  const summaryData = balances.map((balance) => {
    ...
    const initialIncome = previousOps
      .filter((op) => op.type === OperationType.INCOME)
      .reduce((sum, op) => sum + op.amount, 0);
    const initialExpense = previousOps
    ...
    const periodIncome = periodOps
      .filter((op) => op.type === OperationType.INCOME)
      .reduce((sum, op) => sum + op.amount, 0);
    const periodExpense = periodOps
      .filter((op) => op.type === OperationType.EXPENSE)
      .reduce((sum, op) => sum + op.amount, 0);
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>
            Report Period: {format(dateRange.start, 'MMM dd, yyyy')} -{' '}
